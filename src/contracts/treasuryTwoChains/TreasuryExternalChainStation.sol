...
contract TreasuryExternalChainStation is ExternalChainStationStructs, OApp, OAppOptionsType3, AxelarExecutable {
    ...
    function setHostChainAddress(address hostChainStationAddress, string memory hostChainStationAddressString)
        external
        onlyAdmin
    {
    ...
    function quoteLayerZero(address toAddress, address token, uint256 amount) public view returns (uint256) {
        MessagingFee memory fee =
            _quote(layerZero.hostChainStationEid, encodePayload(token, toAddress, amount), _options, false);
    ...
    function decodeAndGive(bytes memory payload) internal {
        (address token, address toAddress, uint256 amount) = decodePayload(payload);
        if (token == address(0)) {
